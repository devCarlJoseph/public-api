...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const steps = await db.orm.public.RecipeStep.where({ recipeId: recipe.id })
    .orderBy((step) => step.stepNumber.asc())
    .all();

  const recipeIngredients = await db.orm.public.RecipeIngredient.where({
    recipeId: recipe.id,
  })
  ...
  const categories = await db.orm.public.RecipeCategory.where({
    recipeId: recipe.id,
  }).all();
  ...
  const diets = await db.orm.public.RecipeDiet.where({
    recipeId: recipe.id,
  }).all();
  ...
  const nutrition = await db.orm.public.RecipeNutrition.where({
    recipeId: recipe.id,
  }).first();
